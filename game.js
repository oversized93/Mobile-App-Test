...
function tickWorld(dt) {
    ...
    if (typeof wind !== 'undefined' && wind) {
        ...
        if (resort.worldClock - resort.__windTick > 20) {
            ...
            // Storm gusts: a passing shower biases the walk upward, so
            // rain arrives with weather instead of on a still day
            const gust = (typeof rainEnvNow !== 'undefined')
                ? rainEnvNow * 1.6 : 0;
            wind.speed = Math.max(0, Math.min(13,
                (wind.speed || 4) + (Math.random() - 0.5) * 2.5 + gust));
        }
    }
...
function drawOverworld() {
    ...
    if (scene3dReady && typeof npcStates !== 'undefined'
        ...
        for (const s of npcStates) {
            ...
            if (la < 0.03) continue;
            ctx.globalAlpha = la;
            const label = ((s.heater || 0) >= 2 ? '\u{1F525} ' : '')
                + (s.name === champName ? '\u{1F451} ' : '') + s.name;
    ...
    if (scene3dReady && typeof windClock !== 'undefined') {
        ...
        let wTxt = rainingNow
            ...
            : (cross && cross <= 300 ? '\u26C5 rain in ' + fmt(cross)
                                     : '\u2600\uFE0F clear skies');
        // Breeze readout: map-fixed arrow + mph, so the lean every ball
        // shows in the air can be read off the chip
        if (typeof wind !== 'undefined' && wind && wind.speed >= 2) {
            const ARR = ['\u2192', '\u2198', '\u2193', '\u2199',
                '\u2190', '\u2196', '\u2191', '\u2197'];
            const ai = Math.round((((wind.angle % (Math.PI * 2))
                + Math.PI * 2) % (Math.PI * 2)) / (Math.PI / 4)) % 8;
            wTxt += '  \u2022  ' + ARR[ai] + ' ' + Math.round(wind.speed);
        }
        ctx.font = 'bold 11px -apple-system,sans-serif';
